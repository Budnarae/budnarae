...
오래된 버전의 C++ 문법에는 throw 키워드를 아래와 같이 사용하는 방법이 있었다.
다음과 같이 throw 키워드를 사용하면 함수가 throw하는 예외 타입을 사전에 제한할 수 있다.
...
단, 컴파일러 최적화 이유로 C++ 11에서 deprecated 되었고, C++ 17에서 완전히 폐기되었다.
대신 새로 추가된 noexcept를 쓰는 게 좋다.

~~~cpp

int Func(int num) noexcept
{
	// . . . .
}
...
noexcept는 throw와 달리, 함수가 예외를 던질지 여부만을 사전에 제한한다.
